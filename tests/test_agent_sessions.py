...
def _client(tmp_path, monkeypatch, limit: int):
    ...
    # Kho phiên gắn vào ROUTER, không phải biến toàn cục — lấy qua đúng đường
    # mà `/api/test/reset` và trang quan sát đang dùng.
    store = api.main.agent_router.sessions  # type: ignore[attr-defined]
    store.limit = limit
