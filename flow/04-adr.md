...
| 3 | ~~**Deploy: vẫn chạy local**~~ → **Deploy lên Render** — sửa 2026-08-08 (C-018). Giả định rủi ro nhất (độ chính xác STT tiếng Việt) CHƯA ĐO ĐƯỢC LẦN NÀO vì cần HTTPS thật trên điện thoại, mà `adb reverse` chỉ dùng được với đúng một máy cắm dây. Đo được nó giờ đáng hơn là trì hoãn. Lý do ban đầu ở dưới | Giả định rủi ro nhất (độ chính xác STT) chưa đo. Deploy trước khi biết app có dùng được không là làm ngược thứ tự | Deploy lên Cloudflare/Vercel ngay: cần HTTPS thật cho micro (đúng là có lợi), nhưng kéo theo quản lý secret + chi phí trước khi biết có đáng không. `adb reverse` đã đủ để test trên điện thoại |
...
| 14 | ~~**Lưu bằng file JSON trên đĩa**~~ → **Postgres khi có `DATABASE_URL`, file khi không** — sửa 2026-08-08 (C-017). Đĩa của Render free là TẠM THỜI: file JSON bị xoá mỗi lần redeploy hoặc ngủ dậy, người dùng mất sạch phiên mà không hề được báo. Vẫn giữ bản file để `npm run dev` chạy được khi không có CSDL. Lý do ban đầu ở dưới (2026-08-08) | Một máy, một người ghi, vài chục phiên. File JSON ghi nguyên khối là đủ và đọc được bằng mắt khi cần soi. Đổi sang SQLite sau chỉ phải thay lớp repository — chữ ký đã có sẵn từ decision 0001 | SQLite ngay: đúng hơn khi nhiều người ghi cùng lúc, nhưng thêm dependency gốc và một lớp migration cho bài toán chưa có |
...
| 15 | ~~**Định danh phiên bằng id trong URL, vẫn KHÔNG auth**~~ → **Phiên gắn với THIẾT BỊ qua header `X-Device-Id`, vẫn KHÔNG auth** — sửa 2026-08-08 (C-019). Bản cũ để `/api/sessions/active` trả phiên đang chơi TOÀN CỤC: chạy local một người thì không thấy gì, nhưng deploy công khai rồi thì hai người mở app là rơi vào chung một ván bài và ghi đè điểm của nhau. Client sinh một UUID lưu localStorage và gửi kèm mọi request; server lọc `active` theo đúng thiết bị đó, thiếu header thì trả rỗng chứ không lộ phiên người khác. Vẫn không có tài khoản: đây là cái vòng tay giữ chỗ, mất nó thì mất đường về phiên cũ — chấp nhận được. Lý do ban đầu ở dưới | Thêm tài khoản: đúng lý do quyết định 2 đã bác — không pain nào trong PRD cần tới, mà thêm cả một bề mặt hỏng. Để id phiên trong URL làm định danh duy nhất: ai có link là vào được (đúng ý), nhưng mở app từ màn hình chính thì không có link nào để mà quay lại |

| 18 | **Trả lời bằng CHỮ, tắt đọc thành tiếng — giữ nguyên đầu vào giọng nói** (2026-08-08) | Operator đo trên điện thoại thật sau khi deploy (C-018): NHẬN giọng nói tiếng Việt thì đạt, nhưng giọng ĐỌC RA sai ngữ điệu. Nghe một câu máy đọc sai giữa ván bài khó chịu hơn là liếc mắt đọc. Đây là TẮT chứ không phải bỏ: `speak()` trong `src/voice/speech.ts` giữ nguyên kèm ghi chú, bật lại chỉ là gọi lại từ `useConversation.ts` | Xoá hẳn phần TTS cho gọn: lần sau có giọng đọc nghe được thì phải viết lại cả đoạn chọn giọng tiếng Việt. Tắt luôn cả micro: sai hướng — chính đầu vào giọng nói mới là thứ đo được là dùng tốt, và là lý do app này tồn tại |
